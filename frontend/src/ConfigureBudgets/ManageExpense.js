 ...
 // const { isLoggedIn } = useAuth();
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const categoriesResponse = await axios.get(`http://54.83.95.43:3002/get-categories/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  const handleAddExpense = async () => {
    try {
      ...
      await axios.post('http://54.83.95.43:3002/add-expense', {
        userId: userId,
        month: selectedMonth,
