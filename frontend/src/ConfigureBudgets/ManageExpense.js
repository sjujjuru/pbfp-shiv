 ...
 // const { isLoggedIn } = useAuth();
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const categoriesResponse = await axios.get(`http://45.55.66.19:3002/get-categories/${userId}?month=${selectedMonth}`, {
          headers: {
  ...
  const handleAddExpense = async () => {
    try {
      ...
      await axios.post('http://45.55.66.19:3002/add-expense', {
        userId: userId,
        month: selectedMonth,
